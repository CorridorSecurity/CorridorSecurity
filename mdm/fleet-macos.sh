...
# Read device configuration from managed plist (pushed via Fleet configuration profile)
FLEET_USER_EMAIL=$(defaults read /Library/Managed\ Preferences/dev.corridor.mdm.plist UserEmail 2>/dev/null || echo "")
FLEET_SERIAL_NUMBER=$(defaults read /Library/Managed\ Preferences/dev.corridor.mdm.plist SerialNumber 2>/dev/null || echo "")
...
# Check if configuration is set. If Fleet did not substitute the
# $FLEET_SECRET_CORRIDOR_TEAM_TOKEN variable (e.g. it is not defined in
# Fleet), the shell expands it to an empty string.
if [ -z "$CORRIDOR_TEAM_TOKEN" ] || [ "$CORRIDOR_TEAM_TOKEN" = "cor-team_..." ]; then
...
DEVICE_SERIAL="$FLEET_SERIAL_NUMBER"
if [ -z "$DEVICE_SERIAL" ]; then
    log_error "Could not retrieve device serial number. Ensure the Fleet configuration profile pushes SerialNumber to /Library/Managed Preferences/dev.corridor.mdm.plist."
    exit 1
fi
log_info "Device Serial: $DEVICE_SERIAL"

# Set the user email from managed plist
USER_EMAIL="$FLEET_USER_EMAIL"
if [ -z "$USER_EMAIL" ]; then
    log_error "Could not retrieve user email. Ensure the Fleet configuration profile pushes UserEmail to /Library/Managed Preferences/dev.corridor.mdm.plist."
    exit 1
fi
